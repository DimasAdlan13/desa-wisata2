<div>
    <div class="max-w-4xl mx-auto px-4 py-12">
        ...
        @else
            {{-- ====== ADMIN / SUPER ADMIN: hanya profil + shortcut ke panel ====== --}}
            <div class="flex flex-col md:flex-row gap-6">

                {{-- Card: Admin Panel Shortcut --}}
                <a href="/admin" class="flex-1 bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 rounded-2xl p-6 hover:shadow-md transition-all group">
                    <div class="text-3xl mb-3">⚙️</div>
                    <h3 class="font-bold text-gray-800 text-lg mb-1 group-hover:text-orange-600 transition">Panel Admin</h3>
                    <p class="text-sm text-gray-500">Kelola booking, layanan, konten, dan pengguna melalui panel admin.</p>
                    <span class="inline-block mt-4 text-xs font-semibold text-orange-600 bg-orange-100 px-3 py-1 rounded-full">Buka Panel →</span>
                </a>
                ...
                <div class="flex-1 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div><span class="text-gray-500 w-28 inline-block">No. HP</span><span>{{ auth()->user()->phone ?? '-' }}</span></div>
                        <div><span class="text-gray-500 w-28 inline-block">Role</span>
                            <span class="bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full text-xs font-medium capitalize">{{ auth()->user()->role }}</span>
                        </div>
                    </div>
                </div>
            </div>
