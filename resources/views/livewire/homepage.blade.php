<div>
    ...
    <section
        ...
        style="height: 68vh; min-height: 420px;"
    >
        @if($latestContents->count() > 0)
            ...
            >
                ...
                <div class="relative shrink-0 w-full h-full">
                    ...
                    <div class="absolute inset-0 flex flex-col justify-end px-6 pb-16 md:px-16 md:pb-20 z-10 max-w-5xl">
                        <span class="inline-block bg-teal-400/90 backdrop-blur text-white text-xs font-bold px-4 py-1 rounded-full uppercase tracking-widest mb-4 w-fit shadow">
                            Info & Artikel
                        </span>
                        <h2 class="text-2xl sm:text-4xl md:text-5xl font-extrabold text-white leading-tight mb-5 drop-shadow-md line-clamp-3">
                            {{ $content->title }}
                        </h2>
            ...
            <button @click="prev()" class="hidden md:flex absolute left-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-black/30 hover:bg-black/50 text-white rounded-full items-center justify-center transition backdrop-blur-sm">‹</button>
            <button @click="next()" class="hidden md:flex absolute right-4 top-1/2 -translate-y-1/2 z-20 w-10 h-10 bg-black/30 hover:bg-black/50 text-white rounded-full items-center justify-center transition backdrop-blur-sm">›</button>
        @else
    ...
    <section class="bg-white py-16 md:py-24 relative overflow-hidden">
        <div class="absolute inset-0 opacity-[0.03]" style="background-image: radial-gradient(circle, #0d9488 1px, transparent 1px); background-size: 28px 28px;"></div>

        <div class="relative max-w-5xl mx-auto px-4 text-center">

            <!-- Headline: animasi CSS murni, selalu jalan saat halaman dimuat -->
            <div class="animate-fade-in-up">
                <span class="inline-block bg-teal-50 text-teal-600 text-xs font-bold px-4 py-1.5 rounded-full border border-teal-200 uppercase tracking-widest mb-5">
                    🏝️ Platform Wisata Terpercaya
                </span>
                <h1 class="text-3xl sm:text-4xl md:text-5xl font-extrabold text-gray-900 leading-tight mb-5">
                    Jelajahi Keindahan<br>
                    <span class="text-teal-600">Kepulauan Seribu</span>
                </h1>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 animate-fade-in-up-delay-1">
                ...
                <div class="bg-gray-50 border border-gray-100 rounded-2xl p-5"
                     ...
                     ">
                    <div class="text-3xl font-extrabold text-teal-600 mb-1" x-text="display + '+'">{{ $stats['total_services'] }}+</div>
                ...
                <div class="bg-gray-50 border border-gray-100 rounded-2xl p-5"
                     ...
                     x-init="
                         ...
                         let t = setInterval(() => { c = Math.min(c + step, target); display = Math.floor(c); if (c >= target) clearInterval(t); }, 16);
                     ">
                    <div class="text-3xl font-extrabold text-teal-600 mb-1" x-text="display + '+'">{{ $stats['total_bookings'] }}+</div>
                ...
                <div class="bg-gray-50 border border-gray-100 rounded-2xl p-5"
                     ...
                     x-init="
                         ...
                         let t = setInterval(() => { c = Math.min(c + step, target); display = Math.floor(c); if (c >= target) clearInterval(t); }, 16);
                     ">
                    <div class="text-3xl font-extrabold text-teal-600 mb-1" x-text="display + '+'">{{ $stats['total_wisatawan'] }}+</div>
