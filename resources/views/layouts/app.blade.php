...
<body class="bg-gray-50 text-gray-800 font-sans antialiased">
    ...
    <nav x-data="{
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <a href="{{ route('home') }}" class="flex items-center gap-2.5 flex-shrink-0" wire:navigate>
                    <img src="{{ asset('images/logo.webp') }}" alt="Logo Desa Wisata"
                        class="h-10 w-auto transition-all duration-300 drop-shadow-sm"
                        :class="scrolled ? 'h-9' : 'h-10'">
                    <div class="hidden sm:block">
                        <span :class="scrolled ? 'text-gray-900' : 'text-gray-800'"
                            class="font-bold text-sm leading-none block transition-colors duration-300">Desa
                            Wisata</span>
                        <span :class="scrolled ? 'text-teal-600' : 'text-teal-500'"
                            class="text-xs leading-none transition-colors duration-300">P. Pramuka</span>
                    </div>
                </a>
                    ...
                    @auth
                        <a href="{{ route('dashboard') }}" wire:navigate
                            ...
                            <div class="w-7 h-7 bg-teal-100 rounded-full flex items-center justify-center">
                                <span
                                    class="text-teal-700 font-bold text-xs">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                            </div>
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button
                                class="text-sm text-gray-400 hover:text-red-500 px-2 py-1.5 rounded-lg hover:bg-red-50 transition-all">
                                Keluar
                            </button>
                        </form>
                    @else
                ...
                <div class="flex md:hidden items-center gap-2">
                    ...
                    @auth
                        <div class="w-8 h-8 bg-teal-100 rounded-full flex items-center justify-center">
                            <span
                                class="text-teal-700 font-bold text-sm">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                        </div>
                    @endauth
                    <!-- Hamburger Button -->
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200"
            ...
            class="md:hidden border-t border-gray-100 bg-white shadow-lg" x-cloak>
            <div class="max-w-7xl mx-auto px-4 py-3 space-y-1">
                ...
                @auth
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button
                            class="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm text-red-500 hover:bg-red-50 transition-all text-left">
                            <span>🚪</span> Keluar
                        </button>
                    </form>
                @else
        ...
        <div class="max-w-7xl mx-auto px-4 grid md:grid-cols-3 gap-8">
            ...
            <div>
                ...
                <ul class="space-y-1 text-sm">
                    <li><a href="{{ route('layanan.index') }}" class="hover:text-white" wire:navigate>Layanan Wisata</a>
                    </li>
                    <li><a href="{{ route('konten.index') }}" class="hover:text-white" wire:navigate>Artikel & Info</a>
                    </li>
            ...
            <div>
                ...
                <p class="text-sm">📧 superadmin56@gmail.com</p>
                <p class="text-sm">
                    📞 <a href="https://wa.me/6285814061431" target="_blank"
                        class="hover:text-green-400 hover:underline transition-colors">085814106159</a>
                </p>
            </div>
        </div>
